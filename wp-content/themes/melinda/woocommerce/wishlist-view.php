			...
			<td colspan="<?php echo esc_attr( $column_count ) ?>" class="cart-lst-el_f">
				<?php if( $show_cb ) : ?>
					<span class="custom-add-to-cart-button-cotaniner">
						<a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_meta['wishlist_token'] ) ) ) ?>" class="button alt" id="custom_add_to_cart"><?php esc_html_e( 'Add the selected products to the cart', 'yith-woocommerce-wishlist' ) ?></a>
					</span> &nbsp;
				<?php endif; ?>
				...
				<?php if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
					<span class="ask-an-estimate-button-container">
						<a
							href="<?php
							if ($additional_info) { echo '#ask_an_estimate_popup'; } else { esc_url($ask_estimate_url); }
							?>"
